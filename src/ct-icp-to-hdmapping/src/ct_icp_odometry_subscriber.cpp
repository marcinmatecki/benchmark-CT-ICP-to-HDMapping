...
int main(int argc, char **argv)
{
    ...
    for (const rosbag::MessageInstance& m : view) {
        if (m.getTopic() == "/ct_icp/world_points") {
            ROS_INFO("Received message on topic: /ct_icp/world_points");
        ...
        if (m.getTopic() == "/ct_icp/pose/odom") {
            ROS_INFO("Received message on topic: /ct_icp/pose/odom");
