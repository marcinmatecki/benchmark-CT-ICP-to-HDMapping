...
# run_benchmark.sh - Reusable benchmark runner for CT-ICP (ROS1)
set -e
...
echo "=========================================="
echo "Running CT-ICP benchmark (ROS1)"
...
IMAGE="${CTICP_IMAGE:-ghcr.io/mapshd/cticp2hdmapping:latest}"
...
docker run --rm -t \
    ...
    bash -lc "
      ...
      roslaunch ${CTICP_LAUNCH} >/tmp/cticp.log 2>&1 &
      LAUNCH_PID=\$!
      ...
      rosrun ct-icp-to-hdmapping listener \"\${REC_BAG}\" \"\${OUT_DIR}/hdmapping\"
    "
